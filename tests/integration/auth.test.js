...
describe('Auth routes', () => {
  ...
  describe('POST /api/v1/auth/forgot-password', () => {
    ...
    test('should return 204 and send reset password email to the user', async () => {
      ...
      await request(app).post('/api/v1/auth/forgot-password').send({ email: userOne.email }).expect(httpStatus.NO_CONTENT);
    ...
    test('should return 400 if email is missing', async () => {
      ...
      await request(app).post('/api/v1/auth/forgot-password').send().expect(httpStatus.BAD_REQUEST);
    });

    test('should return 404 if email does not belong to any user', async () => {
      await request(app).post('/api/v1/auth/forgot-password').send({ email: userOne.email }).expect(httpStatus.NOT_FOUND);
    });
  });
  ...
  describe('POST /api/v1/auth/reset-password', () => {
    ...
    test('should return 400 if reset password token is missing', async () => {
      ...
      await request(app).post('/api/v1/auth/reset-password').send({ password: 'password2' }).expect(httpStatus.BAD_REQUEST);
    });
  ...
  describe('POST /api/v1/auth/send-verification-email', () => {
    ...
    test('should return 401 error if access token is missing', async () => {
      ...
      await request(app).post('/api/v1/auth/send-verification-email').send().expect(httpStatus.UNAUTHORIZED);
    });
  });
  ...
  describe('POST /api/v1/auth/verify-email', () => {
    ...
    test('should return 400 if verify email token is missing', async () => {
      ...
      await request(app).post('/api/v1/auth/verify-email').send().expect(httpStatus.BAD_REQUEST);
    });
